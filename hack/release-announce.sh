...
downloads() {
    ...
    local GHRELURL="https://github.com/ansibleplaybookbundle/import-vm-apb/releases/tag/"
    local RELURL="$GHRELURL$RELREF"
    cat <<EOF
The source code and selected binaries are available for download at:
<$RELURL>.

The primary release artifact of this APB is the git tree. The release tag is
signed and can be verified using [git-evtag][git-evtag].

Pre-built containers are published on Docker Hub and can be viewed at:
<https://hub.docker.com/r/ansibleplaybookbundle/import-vm-apb/>.
EOF
}
...
functest() {
    log "Functional tests are not yet available"
